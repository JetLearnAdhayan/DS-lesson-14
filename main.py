...
#homework
top10_confirmed = pd.DataFrame(data.groupby("Country")["Confirmed"].sum().nlargest(10).sort_values(ascending=False))
fig6 = px.bar(top10_confirmed, x="Confirmed", y=top10_confirmed.index, height=600, color="Confirmed",
              orientation="h", color_continuous_scale=["deepskyblue", "orangered"], title="Top 10 Confirmed Cases Countries")
fig6.write_html("Fig6.html", auto_open=True)

top10_recovered = pd.DataFrame(data.groupby("Country")["Recovered"].sum().nlargest(10).sort_values(ascending=False))
fig7 = px.bar(top10_recovered, x="Recovered", y=top10_recovered.index, height=600, color="Recovered",
              orientation="h", color_continuous_scale=["lightgreen", "darkgreen"], title="Top 10 Recovered Cases Countries")
fig7.write_html("Fig7.html", auto_open=True)

top10_active = pd.DataFrame(data.groupby("Country")["Active"].sum().nlargest(10).sort_values(ascending=False))
fig8 = px.bar(top10_active, x="Active", y=top10_active.index, height=600, color="Active",
              orientation="h", color_continuous_scale=["lightskyblue", "blue"], title="Top 10 Active Cases Countries")
